...
callout('Evidence boundary','All quantitative evidence measures surrogate fidelity to the declared reduced-order simulator. This is not commercial TCAD, fab calibration, or production process prediction.')
...
doc.add_paragraph('The project’s strongest evidence is methodological: declared physics, reproducible simulation data, blocked evaluation, a real baseline, multi-output neural modeling, uncertainty checks, interpretable profile diagnostics, and mandatory simulator verification of inverse-designed recipes. Its limitations are equally explicit, keeping prototype results separate from fab or commercial-TCAD claims.')
